<template>
    <div class="container">
        <div class="my-5 col-12 justify-content-center">
            <h1>{{categories[this.$route.params.id]}}</h1>
        </div>
        <div class="col-12 row mt-5">
            ...
            <div>
                ...
                <b-list-group>
                    ...
                    <b-list-group-item :class="[priceOrder == 2 ?'selected':'d', 'btnOrder']">
                        <div  @click="priceOrder=2" :class="[priceOrder == 2 ?'selected':'d', 'btnOrder','row', 'justify-content-around']">
        ...
        <div class="col-8 mt-5" v-if="services != null && services.length != 0">
        ...
        <div v-else class="col-8 mt-5">
            <h2>{{categories[this.$route.params.id]}}</h2>
...
<script>
    export default {
        ...
        const provinces = await this.$axios.$get('https://apis.datos.gob.ar/georef/api/provincias?orden=nombre&max=5000')
            ...
            .then((res) => {
                this.rawServices = res.data.services.filter((service)=> service.user != this.user && service.category == this.$route.params.id);
                var cats = [
                        'Belleza y cuidado personal',
                        'Servicios de limpieza',
                        'Servicios educativos',
